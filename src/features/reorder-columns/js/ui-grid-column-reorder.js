      ...
      compile: function() {
        return {
          post: function ($scope, $elm, $attrs, uiGridCtrl) {
            if (uiGridCtrl.grid.options.enableColumnReordering && $scope.col.colDef.enableColumnReordering !== false) {
//              $attrs.$observe('renderIndex', function (n, o) {
//                // $log.debug('renderIndex', $scope.$eval(n));
//                $scope.renderIndex = $scope.$eval(n);
//              });

              $compile($elm.attr("ui-grid-column-reorder"))($scope);

//              var columnReorderElm = $templateCache.get('ui-grid/columnReorder');
//
//              var resizerLeft = angular.element(columnReorderElm).clone();
//              var resizerRight = angular.element(columnReorderElm).clone();
//
...
//              $compile(resizerRight)($scope);
  ...
  var module = angular.module('ui.grid.reorderColumns', ['ui.grid']);
  ...
  angular.module('ui.grid.reorderColumns').directive('uiGridColumnReorder', ['$log', '$document', 'uiGridConstants', 'gridUtil', function($log, $document, uiGridConstants, gridUtil) {
    return {
      replace: true,
      priority: -11,//e
